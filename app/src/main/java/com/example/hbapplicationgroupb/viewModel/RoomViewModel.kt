...
import com.example.hbapplicationgroupb.model.updateImage.UpdateImageResponse
...
class RoomViewModel @Inject constructor(
    ...
    private val _checkIfImageUploadSuccessfully: MutableLiveData<UpdateImageResponse> = MutableLiveData()
    val checkIfImageUploadSuccessfully:LiveData<UpdateImageResponse> = _checkIfImageUploadSuccessfully
...
//                    _hotelReview.postValue(Resource.Error(throw(response.errorBody())))
    ...
    fun sendImageToApi(url:String){
        viewModelScope.launch {
            val response = apiRepository.uploadImage(url)
            try {
                if (response.isSuccessful){
                    _checkIfImageUploadSuccessfully.postValue(response.body())
                }
            }catch (e:Exception){
                _checkIfImageUploadSuccessfully.postValue(null)
